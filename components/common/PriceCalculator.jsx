...
export default function PriceCalculator({ 
  ...
  const POS_OBRA_MULTIPLIER = 5/3; // pós-obra: 8h => R$300 cliente, R$250 faxineira
  const serviceMultipliers = {
    padrao: 1,
    com_organizacao: 1.1,
    pos_obra: POS_OBRA_MULTIPLIER
  };
  ...
  // Cleaner earnings: pós-obra pays R$250 when client pays R$300 => share = 250/300
  const cleanerEarnings = serviceType === 'pos_obra' ? basePrice * (250 / 300) : basePrice * (1 - AGENCY_FEE);
            ...
            {(serviceType === 'pos_obra' || serviceType === 'com_organizacao') && (
              <div className="flex justify-between text-sm">
                <span className="text-slate-600 dark:text-slate-400">
                  Multiplicador ({serviceMultipliers[serviceType].toFixed(2)}x)
                </span>
